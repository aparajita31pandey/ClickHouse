...
- `kafka_partition_assignment` — Partition assignment strategy. With the default empty value, partitions are distributed by the Kafka broker through consumer group rebalancing (`subscribe()`). With `shard_sticky`, the partitions listed in `kafka_shard_partitions` are pinned to this table via a client-side `assign()`: no consumer group is joined, no rebalances happen, and restarts of other shards never pause this one. Offsets are still committed to `kafka_group_name` for recovery and lag monitoring. Not supported together with Keeper-based offset storage (`kafka_keeper_path`). Default: empty string.
- `kafka_shard_partitions` — Comma-separated list of partitions owned by this shard, for example `'0,1,2,3'`. Applies to every topic in `kafka_topic_list`. Requires `kafka_partition_assignment = 'shard_sticky'`. Give each shard of a distributed setup a disjoint partition list to get duplicate-free, rebalance-free ingestion. Default: empty string.
- `kafka_replica_consume_mode` — How the consumers of one table (`kafka_num_consumers`) share the partitions from `kafka_shard_partitions`: `cooperative_split` distributes them round-robin so each partition is read by exactly one consumer; `redundant` makes every consumer read all shard partitions (each replica keeps a full copy). Only used with `kafka_partition_assignment = 'shard_sticky'`. Default: `cooperative_split`.
